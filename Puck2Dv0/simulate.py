...
class P2D_env:
    def __init__(self):
        self.x = 1.0 + np.random.normal(0.0, 0.01)
        self.y = 1.0 + np.random.normal(0.0, 0.01)
        self.dx = momentum
        self.dy = momentum
    ...
    def reset(self):
        ...
        self.y = 1.0 + np.random.normal(0.0, 0.025)
        self.dx = momentum
        self.dy = momentum
        self.t = 0
...
# Load BNN
bayes_model = PosteriorModel("Posteriors/BayesDyn_P2D_v0_RUN")

# Load DNN
control_model = PosteriorModel("Posteriors/Control_P2D_v0_RUN", deterministic=True)
...
for traj in trange(25, desc="Simulating the Control Loop"):
    ...
    print("    DONE    ")
np.save("simulated_P2D_v0_%s"%(10), np.asarray(trajectories))
