...
class P2D_env:
    def __init__(self):
        ...
        self.T = 25       # Time for each trajectory
...
bayes_model = opt.compile(model, loss_fn=loss, epochs=35, learning_rate=learning_rate,
                          decay=decay, inflate_prior=0.001, mode='regression', robust_train=0)
...
global_action_sequences = sample_action_sequence(2048, 9)
...
for iteration in range(EPISODES):
    ...
    global_action_sequences = sample_action_sequence(2048, 5)
...
np.save("Controller_Data_X", X_con_train)
np.save("Controller_Data_Y", y_con_train)

# Save BNN
bayes_model.save("Posteriors/BayesDyn_P2D_v0_RUN")
# Save DNN
control_model.save("Posteriors/Control_P2D_v0_RUN")
